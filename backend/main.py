...
app = FastAPI(title="ROI Template OCR Studio v4.2.1 (stable)")
...
@app.get("/api/engines")
def engines():
    # only expose Tess + Rapid (no mention of Paddle)
    return {"tesseract":True, "rapid": ENGINES_STATUS.get("rapid", False), "rapid_error": ENGINES_STATUS.get("rapid_error","")}
...
def api_upload_template(pid: str, file: UploadFile = File(...), page: Optional[int] = Form(default=1)):
    try:
        ...
        meta = storage.load_json(pid, 'project.json')
        if ext == ".pdf":
            img, total_pages, used_page = render_pdf_page_to_bgr(raw_path, page=page or 1, dpi=300)
            png_path = os.path.join(pdir, "template_raster.png")
            cv2.imwrite(png_path, img)
            h, w = img.shape[:2]
            meta.update({"template_path": png_path, "template_pdf": raw_path, "template_pdf_page": int(used_page), "template_width": w, "template_height": h, "template_pdf_pages": int(total_pages)})
            storage.save_json(pid, 'project.json', meta)
            return {"ok": True, "template_path": png_path, "width": w, "height": h, "ext": ".png", "from_pdf": True, "page": int(used_page), "pages": int(total_pages)}
        else:
            from PIL import Image
            with Image.open(raw_path) as im: w,h = im.size
            meta.update({"template_path": raw_path, "template_pdf": None, "template_pdf_page": None, "template_pdf_pages": None, "template_width": w, "template_height": h})
            storage.save_json(pid, 'project.json', meta)
...
def api_template_page(pid: str, page: int = Form(...)):
    try:
        ...
        if not pdf or not os.path.isfile(pdf):
            raise HTTPException(400, "No PDF template stored for this project. Upload a PDF template first.")
        img, total_pages, used_page = render_pdf_page_to_bgr(pdf, page=page or 1, dpi=300)
        png_path = os.path.join(storage.project_dir(pid), "template_raster.png")
        cv2.imwrite(png_path, img)
        h, w = img.shape[:2]
        meta.update({"template_path": png_path, "template_pdf_page": int(used_page), "template_width": w, "template_height": h, "template_pdf_pages": int(total_pages)})
        storage.save_json(pid, 'project.json', meta)
        return {"ok": True, "width": w, "height": h, "page": int(used_page), "pages": int(total_pages)}
    except HTTPException:
        raise
    except Exception as e:
        return JSONResponse(status_code=500, content={"ok": False, "error": str(e)})
        ...
        for uf in files:
            ...
            with open(dst,'wb') as out:
                while True:
                    ...
                    out.write(chunk)
            if ext == ".pdf":
                img, total_pages, used_page = render_pdf_page_to_bgr(dst, page=pdf_page or 1, dpi=300)
                out_name = f"{os.path.splitext(os.path.basename(dst))[0]}-p{int(used_page)}.png"
