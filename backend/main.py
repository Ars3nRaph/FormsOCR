...
app = FastAPI(title="ROI Template OCR Studio v4.1.0 (pan/zoom + PDF page switch + 10MB limit)")
...
def api_upload_template(pid: str, file: UploadFile = File(...), page: Optional[int] = Form(default=1)):
    try:
        ...
        if ext == ".pdf":
            ...
            meta = storage.load_json(pid, 'project.json')
            meta.update({"template_path": raw_path, "template_pdf": None, "template_pdf_page": None, "template_width": w, "template_height": h})
            storage.save_json(pid, 'project.json', meta)
...
def api_template_page(pid: str, page: int = Form(...)):
    try:
        ...
        if not pdf or not os.path.isfile(pdf):
            raise HTTPException(400, "No PDF template stored for this project")
