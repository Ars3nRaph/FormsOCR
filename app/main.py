...
from .utils_pdf import render_pdf_to_image_arrays, render_pdf_page_to_image_bytes, pdf_page_count
from .rate_limit import PLANS

app = FastAPI(title="ROI Template OCR Studio — SaaS v5.0")
...
def register(user: UserCreate, db: Session = Depends(get_db)):
    ...
    db.add(u); db.commit(); db.refresh(u); token = create_access_token({"sub": str(u.id)}); return TokenResponse(access_token=token)
...
def api_new_project(name: str = Form(...), u: User = Depends(get_current_user)):
    pid = storage.new_project(u.id, name); storage.save_json(u.id, pid, 'project.json', {"name": name, "layouts": {}, "template_is_pdf": False, "template_pages": 1})
    return {"ok": True, "project_id": pid}

@app.get("/api/projects/{pid}")
def api_get_project(pid: str, u: User = Depends(get_current_user)):
    try:
        meta = storage.load_json(u.id, pid, 'project.json'); return {"ok": True, "project": meta, "project_id": pid}
...
def api_upload_template(pid: str, file: UploadFile = File(...), u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        if ext not in [".png",".jpg",".jpeg",".pdf"]: raise HTTPException(400,"Formats supportés: PNG/JPG/PDF")

        tpath = os.path.join(pdir, f"template{ext}")
        with open(tpath, 'wb') as out: out.write(file.file.read())

        meta = storage.load_json(u.id, pid, 'project.json')
        meta["template_path"] = tpath; meta["template_is_pdf"] = ext==".pdf"
        if ext==".pdf":
            pages = pdf_page_count(tpath); meta["template_pages"]=pages
            png = render_pdf_page_to_image_bytes(tpath, 0, scale=2.0)
            import cv2, numpy as np; im = cv2.imdecode(np.frombuffer(png, dtype=np.uint8), cv2.IMREAD_COLOR); h,w = im.shape[:2]
        else:
...
def api_template_image(pid: str, pg: int = Query(1, ge=1), u: User = Depends(get_current_user)):
    try:
        ...
        if ext == ".pdf":
            png = render_pdf_page_to_image_bytes(path, pg-1, scale=2.0)
            return FileResponse(io.BytesIO(png), media_type="image/png")
        media = "image/png" if ext==".png" else "image/jpeg"; return FileResponse(path, media_type=media)
    except HTTPException: raise
    except Exception as e: return JSONResponse(status_code=500, content={"ok": False, "error": str(e)})
...
def api_save_layout(pid: str, layout: Layout, page: int = Query(1, ge=1), u: User = Depends(get_current_user)):
    try:
        pdir = storage.project_dir(u.id, pid)
        if not os.path.isdir(pdir): raise HTTPException(404, "Project not found")
        meta = storage.load_json(u.id, pid, 'project.json'); layouts = meta.get("layouts", {})
        layouts[str(page)] = layout.dict(); meta["layouts"] = layouts
        storage.save_json(u.id, pid, 'project.json', meta); return {"ok": True}
    except HTTPException: raise
    except Exception as e: return JSONResponse(status_code=500, content={"ok": False, "error": str(e)})
...
def api_get_layout(pid: str, page: int = Query(1, ge=1), u: User = Depends(get_current_user)):
    try:
        meta = storage.load_json(u.id, pid, 'project.json'); layout = meta.get("layouts", {}).get(str(page))
        if not layout: raise HTTPException(404, "Aucun layout pour cette page")
...
def api_process(pid: str, files: List[UploadFile] = File(...), u: User = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        ...
        if len(files_list) > limits["batch_size"]: raise HTTPException(402, f"Votre plan autorise {limits['batch_size']} documents par lot.")

        import numpy as np, cv2
        rows_all = []
        if meta.get("template_is_pdf", False):
            # Pre-render needed template pages
            tpages = {}
            for pidx in [int(k) for k in meta.get("layouts", {}).keys()]:
                png = render_pdf_page_to_image_bytes(meta["template_path"], pidx-1, scale=2.0)
                tim = cv2.imdecode(np.frombuffer(png, dtype=np.uint8), cv2.IMREAD_COLOR)
                tpages[pidx] = tim
        else:
            tim = cv2.imdecode(np.fromfile(meta["template_path"], dtype=np.uint8), cv2.IMREAD_COLOR)

        for uf in files_list:
            ext=os.path.splitext(uf.filename)[1].lower()
            if ext not in [".png",".jpg",".jpeg",".pdf"]: continue
            images_for_file=[]
            if ext==".pdf":
                ...
                for idx, im in enumerate(pages, start=1): images_for_file.append((f"{os.path.basename(uf.filename)}#p{idx}", im))
            else:
                data=uf.file.read(); im=cv2.imdecode(np.frombuffer(data, dtype=np.uint8), cv2.IMREAD_COLOR)
